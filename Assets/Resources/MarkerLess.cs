...
public class MarkerLess : MonoBehaviour
{
	...
	{
		distanceTextObject.text = "Inicio de GetCoordinates()";
		//while true so this function keeps running once started.
		while (true) {
			// check if user has location service enabled
			if (!Input.location.isEnabledByUser){
				distanceTextObject.text = "User location not Enabled";
				yield break;
			}
			...
			while (Input.location.status == LocationServiceStatus.Initializing && maxWait > 0) {
				//distanceTextObject.text = "Esperando Inicializacion de Localizacion";
			...
			if (maxWait < 1) {
				distanceTextObject.text = "Tiempo de espera exedido";
				distanceTextObject.text = "Time Out";
				yield break;
			}

			// Connection has failed
			if (Input.location.status == LocationServiceStatus.Failed) {
				distanceTextObject.text = "Conexion Fallada";
				yield break;
			} else {
	...
	public void Calc(float lat1, float lon1, float lat2, float lon2)
	{
		...
		panelDist.text = "Distance: " + distance;
	...
	void Start(){
		...
		panelDist.text = "Iniciando....: " + distance;
	...
	void Update(){
		...
		panelDist.text = "Cargando....: " + distance;
        //GetCoordinates();
